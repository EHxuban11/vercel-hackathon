// Browser-side phone detection with LibreYOLO (COCO) via onnxruntime-web.
...
export class PhoneDetector {
  ...
  private busy = false;
  /** which model/backend actually loaded, e.g. "LibreYOLO9-L · WebGPU" */
  info = "";
  ...
  async init(onProgress?: (msg: string) => void) {
    ...
    // LibreYOLO9-L on WebGPU (GPU-accelerated, strongest model), LibreYOLO9-S on WASM fallback
    try {
      onProgress?.("Loading LibreYOLO (95 MB, GPU)…");
      this.session = await ort.InferenceSession.create("/models/libreyolo9l.onnx", {
        executionProviders: ["webgpu"],
      });
      this.info = "LibreYOLO · WebGPU";
    } catch {
      onProgress?.("WebGPU unavailable — loading LibreYOLO (45 MB, CPU)…");
      this.session = await ort.InferenceSession.create("/models/libreyolo9s.onnx", {
        executionProviders: ["wasm"],
      });
      this.info = "LibreYOLO · CPU";
    }
  ...
  async detect(video: HTMLVideoElement, threshold = 0.35): Promise<Detection | null> {
    ...
    try {
      ...
      if (out.dims.length === 3 && out.dims[2] === 6) {
        // LibreYOLO9 end-to-end output: [1, N, 6] rows of x1,y1,x2,y2,score,class
        ...
        // LibreYOLO9 output: [1, 84, 8400] — 4 box coords + 80 class scores per anchor
